...
class StudentForm extends Component {
  constructor(props) {
    super(props)
    this.state = {StudentId: this.props}
  }
  ...
  componentDidMount = async () => {
    ...
    if (StudentId.location.state !== undefined) {
      const MainStudentId = StudentId.location.state.StudentID
      const value = MainStudentId
      console.log(value)
      if (MainStudentId !== '') {
        const apiUrl = `https://studentdatabase-production-3a0c.up.railway.app/student/${value}`
        ...
        this.setState({
          ...
          presentStreetAddress: data.Address,
  ...
  onSubmitForm = async event => {
    ...
    const {studentNumber, errorMsg} = this.state
    ...
    if (
      ...
      const response = await fetch(apiUrl, valueoptions)
      ...
      const searchResult = formattedData.filter(
        eachItem => eachItem.StudentID === studentNumber,
      )
